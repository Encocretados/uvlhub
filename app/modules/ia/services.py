...
class IaService(BaseService):
    ...
    def ia_service(self):
        try:
            ...
            if not user_message:
                return (
                    jsonify({"error": "El campo 'question' no puede estar vacío."}),
                    400,
                )
            if len(user_message) > 10000:  # Ejemplo: Limitar a 10,000 caracteres
                return (
                    jsonify(
                        {
                            "error": "El campo 'question' excede la longitud máxima permitida."
                        }
                    ),
                    400,
                )
            ...
            text_input = dialogflow.TextInput(
                text=user_message, language_code=DIALOGFLOW_LANGUAGE_CODE
            )
